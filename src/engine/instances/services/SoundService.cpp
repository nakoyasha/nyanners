...
namespace Nyanners::Scripting {
    static auto soundDescriptor = ReflectionDescriptorRegistry::instance()->create_registrator([]() {
       ReflectionService::create_descriptor("SoundService", {"Instance"})
        .add_property_chained<SoundService, double, &SoundService::get_volume, &SoundService::set_volume>("GlobalVolume", Number)
        .add_method_anon("play_sound", [](Instances::Instance* instance, lua_State* context) -> int {
...
SoundService::SoundService(): Instance("SoundService") {
    if (const auto result = ma_engine_init(nullptr, &soundEngine); result != MA_SUCCESS) {
        EngineService::panic(std::format("{}, {}", "SoundService initialization failed!", ma_result_to_string(result)));
    }
}

void SoundService::play_sound(const std::string &path) {
    if (const auto result = ma_engine_play_sound(&soundEngine, path.c_str(), nullptr); result != MA_SUCCESS) {
        Core::Logger::log_error(std::format("Failed playing sound {}: {}", path, ma_result_to_string(result)));
    };
}
...
std::string SoundService::ma_result_to_string(const ma_result& result) {
    ...
    switch (result) {
        ...
        case MA_TOO_BIG:                        return "File too large";
        ...
        case MA_AT_END:                         return "End of file";
        ...
        default:
            return std::format("Unknown miniaudio error ({})", static_cast<int>(result));
    }
}
