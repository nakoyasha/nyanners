...
void LibInstance::attach(lua_State *context) {
	...
	lua_pushcfunction(
	  ...
	  [](lua_State *context) {
		  ...
		  if (instance == nullptr) {
			  luaL_error(
			    context, "Cannot create instance because no such instance exists"
			  );
			  return 0;
		  }
...
LibInstance::create_instance(const std::string &className) {
	...
	for (const auto &descriptor : classes | std::views::values) {
		if (
		  const auto &instanceClass = descriptor; instanceClass.name == className
		) {
			return std::dynamic_pointer_cast<Instances::Instance>(
			  instanceClass.construct()
			);
		}
	}
